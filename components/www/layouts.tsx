...
export const ExamplesLayout = async ({
  children,
}: {
    ...
    <div className="container flex-1 items-start border-b">
      <section
        className={cn(
          ...
          !!session ? "md:pb-8 lg:pb-25" : "md:pb-4 lg:pb-10"
        )}
      >
        ...
        {!session && (
          <div className="flex w-full items-center justify-center space-x-4 py-4 md:pb-10">
            <a
              ...
              href="/auth/login?returnTo=/example/dashboard"
            >
              Login to Start
            </a>
          </div>
      ...
      {!!session && <Card className="mb-10">{children}</Card>}
    </div>
  );
};
